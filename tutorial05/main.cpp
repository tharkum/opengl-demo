...
void render_cb (union sigval arg)
{
    ...
    GL_CHECK(glUniform1f(gScaleUniformLoc, sinf(scale)));
...
static void createShaderProgram(GraphicsContext *context)
{
    ...
    GL_CHECK(gScaleUniformLoc = glGetUniformLocation(shaderProgram.program, "gScale"));
    if (gScaleUniformLoc == -1) {
